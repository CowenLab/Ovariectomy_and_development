...
TBCOWEN$is_thigmotaxis = (MWM$strategy == 1)*1
TBCOWEN$is_circling    = (MWM$strategy == 2)*1
...
TBCOWEN <- TABLExF_Full2 %>% group_by(day_cat, Strain, animalID, Age.months.) %>% summarize(mn_thig = mean(is_thigmotaxis), mn_circ = mean(is_circling) , mn_rnd = mean(is_random_path) , mn_scan = mean(is_scanning) , mn_chain = mean(is_chaining) , mn_direct = mean(is_directed_search) , mn_corrected = mean(is_corrected_path ), mn_dir_path = mean(is_direct_path), mn_persev = mean(is_perseverance), mn_CIPL = mean(CIPL_Scores) )

# Strategies from the Konsolaki 2016 paper.
TBCOWEN$escape = TB$mn_thig + TB$mn_rnd
TBCOWEN$local = TB$mn_scan + TB$mn_chain
TBCOWEN$global = TB$mn_dir_path + TB$mn_corrected # Directed search is not included.
# Other metrics.
TBCOWEN$perf = TB$global-TB$escape
TBCOWEN$allocentric = TB$mn_dir_path + TB$mn_corrected + TBCOWEN$mn_direct

# Look at the effect of DAY on performance treating each Strain (OVX vs. Sham)
ggplot(data = TBCOWEN, aes(x = day_cat, y = escape, color = Strain)) + geom_violin() + geom_point(position=position_jitterdodge(dodge.width=0.9))  + facet_wrap(~Age.months.)
ggplot(data = TBCOWEN, aes(x = day_cat, y = local, color = Strain)) + geom_violin() + geom_point(position=position_jitterdodge(dodge.width=0.9))  + facet_wrap(~Age.months.)
ggplot(data = TBCOWEN, aes(x = day_cat, y = global, color = Strain)) + geom_violin() + geom_point(position=position_jitterdodge(dodge.width=0.9))  + facet_wrap(~Age.months.)
ggplot(data = TBCOWEN, aes(x = day_cat, y = perf, color = Strain)) + geom_violin() + geom_point(position=position_jitterdodge(dodge.width=0.9))  + facet_wrap(~Age.months.)
ggplot(data = TBCOWEN, aes(x = day_cat, y = allocentric, color = Strain)) + geom_violin() + geom_point(position=position_jitterdodge(dodge.width=0.9))  + facet_wrap(~Age.months.)
ggplot(data = TBCOWEN, aes(x = day_cat, y = mn_CIPL, color = Strain)) + geom_violin() + geom_point(position=position_jitterdodge(dodge.width=0.9))  + facet_wrap(~Age.months.)

# What correlates the best with CIPL?
ggplot(data = TBCOWEN, aes(x =perf, y = mn_CIPL, color = Strain)) + geom_point()+ facet_wrap(~Age.months.) +  geom_smooth()
ggplot(data = TBCOWEN, aes(x =allocentric, y = mn_CIPL, color = Strain)) + geom_point()+ facet_wrap(~Age.months.) +  geom_smooth()
ggplot(data = TBCOWEN, aes(x =escape, y = mn_CIPL, color = Strain)) + geom_point()+ facet_wrap(~Age.months.) +  geom_smooth()

# Is there a general effect of age?
ggplot(data = TBCOWEN, aes(x = factor(Age.months.), y = mn_CIPL, color = Strain)) + geom_violin() + geom_point(position=position_jitterdodge(dodge.width=0.9))  + facet_wrap(~day_cat)
ggplot(data = TBCOWEN, aes(x = factor(Age.months.), y = perf, color = Strain)) + geom_violin() + geom_point(position=position_jitterdodge(dodge.width=0.9))  + facet_wrap(~day_cat)
# Same question but use a regression/scatter plot. Should be a correlation.
ggplot(data = TBCOWEN, aes(x = Age.months., y = mn_CIPL, color = Strain)) + geom_point() + facet_wrap(~day_cat) +  geom_smooth()
