...
filename = 'G:/Dropbox/Foldershare/Src/R/MWM/MWM Master Sheet.csv'
TABLExF_Full <- read.csv(filename)
TABLExF_Full$Strain = factor(TABLExF_Full$Strain)
TABLExF_Full$name = factor(TABLExF_Full$name)
...
ggplot(data = TABLExF_Full, aes(x = factor(Age.months),  name, color = Strain)) + geom_boxplot() + facet_wrap(~name,  ncol=1)
